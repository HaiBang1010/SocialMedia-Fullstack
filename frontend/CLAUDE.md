...
## Phase Polish Round 2 — DONE

- **Avatar upload**: dep `react-easy-crop`. `lib/cropAvatar.ts` `getCroppedAvatarBlob(src, area)` (canvas → 512² JPEG q0.9). `components/profile/AvatarUploadDialog.tsx` (file pick jpeg/png/webp ≤5MB → `<Cropper aspect=1 cropShape=round>` + zoom slider → `mediaApi.presign('image/jpeg')` → `uploadToPresignedUrl` → `usersApi.updateMe({avatarUrl})` → `notifySuccess`; fail → `notifyError` + dialog stays). `UserProfilePage` edit-view: avatar section "Change photo" (dialog) + "Reset to default" (inline confirm → `updateMe({avatarUrl:''})` → BE regen DiceBear); `onAvatarSaved` = `updateUser` + invalidate `user(username)`.
- **Suggested follows**: `useSuggestedUsers` (1 query limit 10, 5-min stale, `enabled isAuthenticated`), `useFollowSuggested` (optimistic remove khỏi list + invalidate suggested), `SuggestedUserCard` (`compact` row / card + `onFollowed`), `EmptyFeedSuggestions` (grid; `onboarding` mode + Done). **RightRail refactor** static→`useSuggestedUsers().slice(0,5)` (skeleton; dọn orphan).
- **Mixed feed + onboarding + stranger-follow**: BE trả followed (`isFollowingAuthor:true`) + stranger (`false`) trong 1 feed; cursor opaque (`f:`/`s:`) → FE truyền verbatim (no pagination change). `PostCard`: nút **Follow** khi `!post.isFollowingAuthor && author.id!==meId` → `useFollowAuthor` (on success `markAuthorFollowedInCaches` patch isFollowingAuthor khắp feed cache → button biến mất **no refetch**; invalidate user(username)+suggested). `lib/postCache.markAuthorFollowedInCaches` + `mapPostsByAuthorInInfinite`. **FeedPage onboarding gate**: `useUserProfile(me).followingCount===0 && !onboardingDone` → `<EmptyFeedSuggestions onboarding onDone>` (Done hiện sau ≥1 follow) → Done = `onboardingDone=true` + invalidate feed+user(me).
