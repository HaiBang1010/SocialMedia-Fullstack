...
| POST | `/conversations/:id/messages` | ✓ | gửi message — text caption AND/OR `media[]` 1–10 ảnh+video (mix); contentType DERIVED server-side; participant → **403** else; bump `lastMessageAt` + sender `lastReadMessageId` (Phase 5.1 text → 5.4a media) |
