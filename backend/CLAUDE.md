...
| POST | `/calls/start` | ✓ | start call — body `{conversationId, type AUDIO\|VIDEO}`; participant→**403**, self-only→**400**, active call (convo HOẶC initiator)→**409**; tạo `Call`+CALL message+LiveKit room → 201 `{message, token, url}` (Phase 6) |
...
>
> **Calls (Phase 6 — LiveKit Cloud)**: migration **`add_calls`** — model `Call` (`id` = LiveKit room name [Decision Q9, KHÔNG cột `livekitRoomName`], `conversationId`/`initiatorId` FK, `type CallType`, `startedAt`, `endedAt?`, `endedReason CallEndReason?`, `@@index([conversationId, startedAt])`) + enums `CallType{AUDIO VIDEO}`/`CallEndReason{COMPLETED MISSED DECLINED FAILED}` + `MessageContentType` +`CALL` (`ALTER TYPE ADD VALUE`, PG16 in-txn, precedent voice/sticker) + **`Message.callId String?` FK `onDelete: SetNull`** + back-relations `Conversation.calls`/`User.initiatedCalls`. **Call-as-Message (Decision 1)**: mirror 5.4c sharedPost infra byte-for-byte ⇒ pagination/preview/realtime(`message:new`)/optimistic miễn phí. `messageInclude.call: {include:{initiator}}` + `serializeMessage` narrow `call` card (id/type/startedAt/endedAt/endedReason/initiator; null cho non-CALL) + **`conversationInclude.messages.include.call` PARITY** (type-parity lesson lần 5). `recallMessage` **block CALL** (400 CannotRecallCall — events không retractable). **`lib/livekit.ts`**: dynamic `import('livekit-server-sdk')` (SDK ESM-only, BE compile CommonJS — static import → ERR_REQUIRE_ESM trên Node<22.12; route qua helper để cache giữ ESM-mode type, tránh node16 cjs/esm clash); `generateAccessToken` (TTL 1h, grant roomJoin+publish+subscribe, `toJwt()` async v2), `createCallRoom` (maxParticipants 50 + emptyTimeout 600, soft-fail), `getRoomParticipantCount` (room-not-found→0). **Module `calls/`**: `createCall` (participant gate 403 / self-only 400 / concurrent 409 [`Call where endedAt null AND (conversationId OR initiatorId)`] → create Call + room + CALL message [bump lastMessageAt + initiator lastReadMessageId] → `emitNewMessage`+`emitCallIncoming` → token), `getCallAccessToken` (403/410/409-CallFull via listParticipants≥50), `endCall(action, reason?)` (DIRECT/end_for_all → finalizeCall; GROUP leave → auto-end khi `getRoomParticipantCount ≤1`; idempotent; `end_for_all` GROUP initiator-only 403), `declineCall` (DIRECT finalize DECLINED + emit cả `call:ended`+`call:declined`; GROUP emit `call:declined`). **Socket io.ts** 3 emit helper (mirror emitNewMessage user-room loop): `emitCallIncoming(participantIds, {callId,conversationId,type,isGroup,initiator,conversationName})` [trừ initiator], `emitCallDeclined(initiatorId, …)`, `emitCallEnded(participantIds, …)`. **KHÔNG `call:offer/answer/ice`** (LiveKit lo signaling), **KHÔNG webhook** (defer — missed/ended từ client + LiveKit emptyTimeout backstop), **KHÔNG `getCallHistory`** (entries trong messages list). env +`LIVEKIT_URL/_API_KEY/_API_SECRET` (Zod). OpenAPI **37→41** (calls module 14, register SAU messages để $ref Message; +tag Calls). Smoke: migration applied + `tsc` 0 lỗi + buildOpenApiDocument 41 paths + token mint (288-char JWT) + server boot OK. Browser-verified + follow-up fixes (T1/T2/Issue 1-3/block-react) applied; **Phase 6 COMPLETE**.
>
> **Phase 6 follow-up (Issue 1 — 409 ghost-block)**: `createCall` thêm **stale-lock detection** trước throw 409 — `getRoomParticipantCount(active.id)`: empty(0) AND `age > STALE_CALL_MS`(60s, > connect+ring) → `finalizeCall('FAILED')` reap ghost → continue (self-heal vì KHÔNG end-on-disconnect: webhook defer + FE KHÔNG pagehide handler); else 409 kèm data. **`AppError` +optional `data` arg** (error handler spread; backward-compat) → 409 `CallInProgress` carry `{callId, conversationId, type, isGroup}` để FE offer "Join" thay vì block câm. (Fix A+B; FE C = JoinCallDialog. Skip D pagehide — emptyTimeout 600s + stale-lock đủ.)
>
> **Phase 6 follow-up round 2 (3 issues)**: (1) call hang khi peer close → fix FE-only (client last-participant detection, xem frontend/CLAUDE.md). (2) **`finalizeCall` +`deleteRoom(callId)`** (`lib/livekit.deleteRoom` NEW soft-fail) — LiveKit force-disconnect mọi participant còn lại (onDisconnected→leave) ở MỌI finalize path; fix Issue 2 (end_for_all trước chỉ flip DB + socket, KHÔNG kick LiveKit) + hardening end-of-call. (3) **`activeCall` trong conversation DTO**: `conversationInclude.calls` (where endedAt null, take 1, +initiator) + `serializeConversation.activeCall` (CallInfo shape, endedAt/endedReason null) + `conversationResponseSchema.activeCall` (reuse `callResponseSchema`) — cho banner "Call in progress · Join". OpenAPI vẫn 41 paths.
>
