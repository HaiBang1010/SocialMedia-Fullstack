...
| Polish R2 — Avatar upload (`react-easy-crop` + `cropAvatar` 512² + `AvatarUploadDialog` presign→PATCH, empty `avatarUrl`→DiceBear regen) + Suggested follows (`GET /users/suggested` FoF `groupBy`+popular fallback, RightRail wire real, OpenAPI 47→48) + **Mixed feed** (`getFeed` rewrite: followed `f:<id>` → ranked stranger fill `s:<offset>`, FoF→engagement→recency in-memory pool cap 100, `FEED_DAYS` 14→90, reuse `isFollowingAuthor`) + onboarding gate (`followingCount===0` → grid+Done) + stranger inline-Follow (`useFollowAuthor` patch cache no-refetch) | ✅ Xong (browser-verify do user) |

Project đã hoàn thành 7/7 phase + **Polish Round 1** (Toast / Safari voice / Reply-to / httpOnly cookie) + **Polish Round 2** (avatar upload / suggested follows / mixed feed + onboarding + stranger-follow). Task mới ngoài scope đã làm — hỏi user trước khi mở rộng.
